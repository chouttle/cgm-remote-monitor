...
daytoday.prepareHtml = function daytodayPrepareHtml(sorteddaystoshow) {
  ...
  $('#daytodaycanvases').html('');
  sorteddaystoshow.forEach(function eachDay(d) {
    $('#daytodaycanvases').append($('<table><tr><td><div id="daytodaycanvas-' + d + '"></div></td><td><div id="daytodaystatcanvas-' + d + '"></td></tr></table>'));
  });
};
...
//            .text((t.percent ? (t.percent > 0 ? '+' : '') + t.percent + '%' : '') + (t.absolute ? Number(t.absolute).toFixed(2) + 'U' : ''));
    ...
    charts = d3.select('#daytodaycanvas-' + day).html(
        '<b>'+
        report_plugins.utils.localeDate(day)+
        '</b><br>'
    ).append('canvas')
    ...
    canvg(document.getElementById('canvas-' + day), charts[0][0].outerHTML);
  }
